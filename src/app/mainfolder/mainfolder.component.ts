...
if(!this.activeTabs.includes(tab.name)){
...
close(tab:any){
  ...
  let i = this.activeTabs.indexOf(tab)
  if (i !== -1) {
    this.activeTabs.splice(i, 1);
  }
  if(this.activeTabs.length >= 1 && tab === this.activeSection){
    this.activeSection = this.activeTabs[this.activeTabs.length - 1]
  }
 ...
 consoleText(words: string[], id: string, colors: string[] | undefined = ['#fff']) {
  let visible = true;
...
typingCode: string = '';
cssCode: string = `  /* Add your CSS code here */
body {
     fName: Krishna #fff;
     lName: Karmyal #f5f5f5;
},
.Qualification{
        ssc: PEMS;
        HSC: Dr. D. Y. Patil;
        BSC (comp sci.): DYP ACS Pimpri Pune
}
.Hobbies{
         playing: football;
         other: Coding !important;
}
...
typeCode() {
  ...
  let currentCode = ''
  // const codeLength = this.cssCode.length;

  // for (let i = 0; i < codeLength; i++) {
  //   setTimeout(() => {
  //     currentCode += this.cssCode[i];
  //     this.typingCode = currentCode;
  //   }, 50 * i);
  // }
  const codeLength = this.cssCode.length;
  ...
  for (let i = 0; i < codeLength * 10; i++) {
     setTimeout(() => {
      const index = i % codeLength;
      currentCode = this.cssCode.substring(0, index + 1);
      this.typingCode = currentCode;
    }, 50 * i);
  }
